...
function Basket() {
  ...
  const getdata  = async()=>{
    const res = await axios.get("http://localhost:4000/API/basket")
  ...
  function handledelete(id,Price,price_off,productID){
    axios.delete(`http://localhost:4000/API/basket/${id}`)
    setdata(data.filter(x=>x._id !== id))  
    axios.put(`http://localhost:4000/API/products/${productID}`,{colorforbasket:"black"})       
