import './Favoriler.css'
import React, { useEffect, useState } from 'react'
import axios from 'axios'
import { FaSearch , FaTrash , FaEye} from 'react-icons/fa';
import { AiFillStar} from 'react-icons/ai';
import PulseLoader from "react-spinners/PulseLoader";
import { useNavigate } from 'react-router-dom';
import {Helmet} from "react-helmet";
import Aos from 'aos'
import "aos/dist/aos.css"

function Favoriler() {
  const [data,setdata] = useState([])
  const [productdata,setproductdata] = useState([])
  
  const [Search,setSearch] = useState("")
  const [loading,setloading] = useState(false)

  const [id,setid] = useState([])

  const [items, setItems] = useState([]);
  const navigate = useNavigate()

  useEffect(()=>{
    setloading(true) 
    setTimeout(()=>{
      setloading(false)
    },1000)
  },[])

  const getdata  = async()=>{
    const res = await axios.get("http://localhost:4000/API/favorite")
    setdata(res.data.filter(x=>x.userid === JSON.parse(localStorage.getItem('id'))))
  }
  useEffect(()=>{
      getdata()
  },[])

 
  function remove(id,productID){
    axios.delete(`http://localhost:4000/API/favorite/${id}`)
    setdata(data.filter(x=>x._id !== id))
    axios.put(`http://localhost:4000/API/products/${productID}`,{colorforfav:"white"}) 
  }
 
  function detail(productID){
    localStorage.setItem('detailsproductid',JSON.stringify(productID))
    navigate(`/Details/${productID}`)
  }
  useEffect(() => {
  Aos.init({duration:2000}) 
  },[])
  return (
    <>
    <Helmet>
      <meta charSet="utf-8" /> 
      <title>Favorilər</title>
      <link rel="shortcut icon" href="data:image/jpeg;base64," />
  </Helmet>
    {
      loading ? 
      <PulseLoader className='loader' style={{width:'100%',height:'100vh',zIndex:'100',backgroundColor:'white',position:'fixed',display:'flex',justifyContent:'center',paddingTop:'300px'}} color={'green'}  size={25}/>:
      <div className="Favoriler">
          <div className="Kategoriyahead">
            <p className="Kategoriyaheadtext">Favorilər</p>
            <div className="Kategoriyasearch">
                <input onChange={(e)=>{setSearch(e.target.value)}} type="text" className="Kategoriyasearchinput" placeholder='Search'/>
                <FaSearch className='Kategoriyasearchicon'/>
            </div>
          </div>
          <div className="Favorilerheadcontainer">
            
            <div className="Favorilercontainer" data-aos="fade-up">
            {(data)
            .filter(element=> element.keyforSearch.toLowerCase().includes(Search.toLowerCase()))
            .map((item)=>(
                <div key={item._id} className="Favorilercart">
                  <div className="Favorilercartimage">
                    {
                      (item.new === true)? (
                        <div className="moreselloff">Yeni</div>
                      ):null
                    }
                    <img src={item.imageurl} alt="" className="Favorilercartimg" />
                  </div>
                  <div className="Favorilercarttext">
                    <p className="Favorilercarttitletext">{item.title}</p>
                    <p className="Favorilercartabouttext">{item.About}</p>
                    <br/><br/>
                    <div className="Favorilerdivprice">
                    {
                    (item.price_off !== null)? (
                      <div className="Favorilerdivprice">
                        <del className="Favorilercartprice" >{item?.price}</del>
                        <p className="Favorilercartpriceoff">{item?.price_off}</p>
                        manat
                      </div>
                    ):(
                      <div className="Favorilerdivprice">
                        <p className="Favorilercartprice" >{item?.price}</p>
                        manat
                      </div>
                    )                  
                  }
                    </div>
                  </div>
                  <div className="Favorilercarticon">
                      <FaEye onClick={() => detail(item.productID)} className="Favoricarticoneye" />
                      <FaTrash onClick={()=> {remove(item._id,item.productID)}} className='Favorilercarticonheart'/>
                  </div>
                </div>
            ))}
            </div> 
          </div>
      </div>
    }
    </>
  )
}
 
export default Favoriler